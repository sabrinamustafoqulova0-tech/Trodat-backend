...
def test():
    ...
    # Regex that allows backslashes so we can capture JSON-escaped slashes (e.g., \/)
    urls = re.findall(r'https?://[^\s"\'>}]+?\.(?:jpg|png|jpeg|webp)', html, re.IGNORECASE)
    print(f"Total raw URLs matching regex: {len(urls)}")
    ...
    filtered = []
    for u in urls:
        # Unescape slashes and remove backslashes
