...
function getCandidates(stamp) {
  ...
  const nameLower = stamp.name.toLowerCase();
  const list = [];

  // General thestampmaker templates
...
async function run() {
  ...
  for (let i = 0; i < remainingStamps.length; i++) {
    ...
    console.log(`\n[${i+1}/${remainingStamps.length}] Finding direct image for ${stamp.article} (${stamp.name})...`);
    ...
    for (const url of candidates) {
      ...
      try {
        ...
        // Ensure size is > 10KB (a good quality file)
        if (stats.size > 10000) {
          if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
          fs.renameSync(tempPath, filePath);
          console.log(`  ✅ SUCCESS: Downloaded from ${url} (${Math.round(stats.size/1024)} KB)`);
          downloaded = true;
          successCount++;
          break;
        } else {
          // File is too small (e.g. placeholder, thumbnail or 404 image)
      ...
      await sleep(100); // short delay between candidates
    }
    
    if (!downloaded) {
      console.log(`  ❌ Failed to download a high-res direct image.`);
    }
    await sleep(400); // politeness delay between stamps
  }
